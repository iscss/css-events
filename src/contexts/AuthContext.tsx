...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signUp = async (email: string, password: string, fullName?: string) => {
    ...
    if (!rateLimitCheck.isAllowed) {
      return { error: { message: rateLimitCheck.message || 'Too many signup attempts', status: 429 } };
    }
  ...
  const signIn = async (email: string, password: string) => {
    ...
    if (!rateLimitCheck.isAllowed) {
      return { error: { message: rateLimitCheck.message || 'Too many login attempts', status: 429 } };
    }
  ...
  const requestPasswordReset = async (email: string) => {
    ...
    if (!rateLimitCheck.isAllowed) {
      return { error: { message: rateLimitCheck.message || 'Too many password reset attempts', status: 429 } };
    }
  ...
  const value: AuthContextType = {
    user, session, loading, signUp, signIn, signOut, requestPasswordReset, resetPassword
  };
